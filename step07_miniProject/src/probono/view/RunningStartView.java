...
public class RunningStartView {
	...
	public static void main(String[] args) {
		...
		try {
			project = service.getProbonoProject("오드리햅버 프로젝트");
		} catch (NotExsitException e) {
			e.printStackTrace();
		}
		...
		try {
			project2 = service.getProbonoProject("오드리햅버 프로젝트");
		} catch (NotExsitException e) {
			e.printStackTrace();
		}
		...
		System.out.println("\n***** 기부자 수정 후 해당 Project 검색 *****");
		//없는 이름 검색
		Activist newAudreyException = new Activist("newGiver", "나미용", "newPw3", "무상파마지원");
		try {
			service.projectUpdate("오드리햅버 프로젝트aaa", newAudreyException);
		} catch (NotExsitException e) {
			e.printStackTrace();
		}		
		ProbonoProject project3 = null;
		try {
			project3 = service.getProbonoProject("오드리햅버 프로젝트aaa");
		} catch (NotExsitException e) {
			e.printStackTrace();
		}
